...
use BlackCat\Database\Packages\Books\Repository\BookRepositoryInterface;
...
class BookRepository implements BookRepositoryInterface, RepoContract, KeysetRepoContract
{
use OrderByTools, PkTools, RepositoryHelpers;
    ...
    public function upsertMany(array $rows): int {
        ...
        if ($helperKeys && class_exists(\BlackCat\Database\Support\BulkUpsertHelper::class)) {
          $bulk = new \BlackCat\Database\Support\BulkUpsertHelper($this->db, \BlackCat\Database\Packages\Books\Definitions::class);
          $bulk->upsertMany($rows, $helperKeys, [ 'title', 'short_description', 'full_description', 'price', 'currency', 'author_id', 'main_category_id', 'isbn', 'language', 'pages', 'publisher', 'published_at', 'sku', 'is_active', 'is_available', 'stock_quantity', 'deleted_at' ]);
          return count($rows);
        }
    ...
    public function getByTenantIdAndIsbn(int $tenantId, string $isbn, bool $asDto = false): array|\BlackCat\Database\Packages\Books\Dto\BookDto|null {
        return $this->getByUnique([ 'tenant_id' => $tenantId, 'isbn' => $isbn ], $asDto);
    }
    ...
    public function getIdByTenantIdAndIsbn(int $tenantId, string $isbn) {
        $row = $this->getByTenantIdAndIsbn($tenantId, $isbn, false);
        if (!is_array($row)) { return null; }
        return $row['id'] ?? null;
    }
    /** @return array<string,mixed>|\BlackCat\Database\Packages\Books\Dto\BookDto|null */
