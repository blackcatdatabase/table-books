...
namespace BlackCat\Database\Packages\Books\Repository;

use BlackCat\Core\Database as Database;              // e.g. BlackCat\Core\Database
use BlackCat\Database\Packages\Books\Definitions;
use BlackCat\Database\Packages\Books\Criteria;
use BlackCat\Database\Packages\Books\Dto\BookDto as Dto;
use BlackCat\Database\Packages\Books\Mapper\BookDtoMapper as RowMapper;
...
final class BookRepository implements RepoContract, KeysetRepoContract
{
     ...
     */
    protected function def(): string { return \BlackCat\Database\Packages\Books\Definitions::class; }
    ...
    public function upsertMany(array $rows): int {
        ...
        if (class_exists(\BlackCat\Database\BulkUpsertRepository::class) && [ 'tenant_id', 'slug_ci' ] !== []) {
            $bulk = new \BlackCat\Database\BulkUpsertRepository($this->db, \BlackCat\Database\Packages\Books\Definitions::table());
            return $bulk->upsertMany($rows, [ 'tenant_id', 'slug_ci' ], [ 'title', 'short_description', 'full_description', 'price', 'currency', 'author_id', 'main_category_id', 'isbn', 'language', 'pages', 'publisher', 'published_at', 'sku', 'is_active', 'is_available', 'stock_quantity', 'deleted_at' ]);
        }
    ...
    */
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\Books\Dto\BookDto|null */
    public function getByTenantIdAndSlugCiAndIsLive(int $tenantId, string $slugCi, int $isLive, bool $asDto = false): array|\BlackCat\Database\Packages\Books\Dto\BookDto|null {
        $row = $this->getByUnique([ 'tenant_id' => $tenantId, 'slug_ci' => $slugCi, 'is_live' => $isLive ]);
        if (!$asDto || !$row) return $row;
        return \BlackCat\Database\Packages\Books\Mapper\BookDtoMapper::fromRow($row);
    }
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\Books\Dto\BookDto|null */
    public function getByTenantIdAndIsbn(int $tenantId, string $isbn, bool $asDto = false): array|\BlackCat\Database\Packages\Books\Dto\BookDto|null {
        $row = $this->getByUnique([ 'tenant_id' => $tenantId, 'isbn' => $isbn ]);
        if (!$asDto || !$row) return $row;
        return \BlackCat\Database\Packages\Books\Mapper\BookDtoMapper::fromRow($row);
    }
    public function existsByTenantIdAndIsbn(int $tenantId, string $isbn): bool {
        $where = 't.' . Ident::q($this->db, 'tenant_id') . ' = :uniq_tenant_id' . ' AND ' . 't.' . Ident::q($this->db, 'isbn') . ' = :uniq_isbn';
        return $this->exists($where, [ 'uniq_tenant_id' => $tenantId, 'uniq_isbn' => $isbn ]);
    }
    /** @return int|string|null */
    public function getIdByTenantIdAndIsbn(int $tenantId, string $isbn) {
        $row = $this->getByTenantIdAndIsbn($tenantId, $isbn);
        return $row ? ($row['id'] ?? null) : null;
    }
    /** @return array<string,mixed>|\BlackCat\Database\Packages\Books\Dto\BookDto|null */
    public function getByTenantIdAndId(int $tenantId, int $id, bool $asDto = false): array|\BlackCat\Database\Packages\Books\Dto\BookDto|null {
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\Books\Dto\BookDto|null */
    public function getById(int $id, bool $asDto = false): array|\BlackCat\Database\Packages\Books\Dto\BookDto|null {
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\Books\Dto\BookDto|null */
    public function getByIdForTenant(int|string $id, int|string $tenantId, bool $asDto = false): array|\BlackCat\Database\Packages\Books\Dto\BookDto|null {
